        ...
        <div class="col-md-8">
          ...
          <div class="w-full d-flex justify-content-between ">
            <div class="d-flex">
              ...
              @else
                <img src="{{ asset('images/default.jpg') }}" width="52px" height="52px" style="border-radius: 50%"
                  alt="">
              @endif
              <div>
              ...
              @if (Auth::check())
                @if (Auth::user()->role == 'koki' && Auth::user()->id != $data->user->id)
                  <button type="submit" class="btn zoom-effects text-light btn-sm rounded-circle p-2"
                    style="background-color:#F7941E ;" data-toggle="modal" data-target="#exampleModalCenter">
          ...
          <div class="card p-3" id="replies{{ $row->id }}">
            ...
            <div class="collapse" id="collapse{{ $row->id }}">
              <div class="card card-body mx-3">
                <form action="{{ route('ReplyComment.store', $row->id) }}" id="formBalasKomentar{{ $row->id }}"
                  method="POST">
                  ...
                  <div id="cardReplyComment{{ $item->id }}">
                    <div class="user d-flex flex-row mb-2">
                      @if ($item->userSender->foto)
                        <img src="{{ asset('storage/' . $item->userSender->foto) }}" width="30" height="30"
                          class="user-img rounded-circle mr-2">
                      @else
                        <img src="{{ asset('images/default.jpg') }}" width="30" height="30"
                          class="user-img rounded-circle mr-2">
                        ...
                        @if (auth()->check())
                          ...
                          @else
                            <form action="{{ route('replyComment.destroy', $item->id) }}" method="POST"
                              id="replyDelete{{ $item->id }}">
                              @csrf
                              @method('DELETE')
                              <button type="submit" onclick="destroyReplyComment({{ $item->id }})"
                    ...
                    <div class="collapse" id="collapses{{ $item->id }}">
                      ...
                      <form action="{{ route('ReplyReplyComment.store', $row->id) }}" method="POST"
                        id="formBalasBalasKomentar{{ $item->id }}">
                        @csrf
            ...
            <div class="modal-content">
              ...
              <form action="{{ route('report.reply', $row->id) }}" id="FormReportReply{{ $row->id }}"
                ...
                <div class="modal-body d-flex align-items-center" style="background-color: #ffffff;">
                  <!-- Tambahkan kelas "align-items-center" -->
                  @if ($row->user->foto)
                    <img class="rounded-circle" src="{{ asset('storage/' . $row->user->foto) }}" width="106px"
                      height="104px"
                ...
                <div class="modal-footer">
                  <button type="submit" class="btn text-light" onclick="ButtonReportReply({{ $row->id }})"
                    style="border-radius: 15px; background-color:#F7941E; box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25);"><b
                      class="ms-2 me-2">Laporkan</b></button>
                </div>
              </form>
      ...
      @foreach ($replies as $row)
        @if (!empty($row->id))
          <div class="modal fade" id="blockModal{{ $row->id }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                <form action="{{ route('ReplyDestroy.destroy', $row->id) }}" id="FormReplyDestroy${{ $row->id }}"
                  method="POST">
                  @csrf
                  @method('DELETE')
                  <div class="modal-header">
              ...
              <div class="modal-content">
                ...
                <form action="{{ route('report.reply.comment', $item->id) }}" method="POST"
                  ...
                  <div class="modal-body d-flex align-items-center" style="background-color: #ffffff;">
                    <!-- Tambahkan kelas "align-items-center" -->
                    @if ($item->user->foto)
                      <img class="rounded-circle" src="{{ asset('storage/' . $item->user->foto) }}" width="106px"
                        height="104px"
                        style="border-radius: 50%; max-width:110px; border:0.05rem solid rgb(185, 180, 180);"
                        alt="">
                      <textarea class="form-control" name="description" style="margin-left: 1em; border-radius: 15px;" rows="5"
                  ...
                  <div class="modal-footer">
                    <button type="submit" class="btn btn-light text-light"
                      ...
                      style="border-radius: 15px; background-color:#F7941E;"><b class="ms-2 me-2">Laporkan</b></button>
                  </div>
                </form>
              </div>
            </div>
