    ...
    @section('content')
      ...
      <section>
                        ...
                        <div class="modal-content">
                          ...
                          <form action="{{ route('report.course', $detail_course->id) }}" method="post">
                            ...
                            <div class="modal-body d-flex align-items-center">
                              ...
                              <img class="me-2" src="{{ asset('storage/' . $detail_course->foto_kursus) }}"
                    ...
                    @if (Auth::check())
                      @if (Auth::user()->isFavoriteCourse($detail_course->id))
                        <button type="submit" class="btn text-dark mx-1"
                          style="border: 1px solid #000; border-radius: 15px;"><b class="ms-3 me-3"><i id="bookmark"
                              onclick="favoriteKursus()" class="fa-regular fa-solid fa-bookmark"></i></b></button>
                      @else
                        <button type="submit" class="btn text-dark mx-1"
                          style="border: 1px solid #000; border-radius: 15px;"><b class="ms-3 me-3"><i id="bookmark"
                              onclick="favoriteKursus()" class="fa-regular fa-bookmark"></i></b></button>
                      @endif
                    @else
                      <button type="submit" class="btn text-dark mx-1"
                        style="border: 1px solid #000; border-radius: 15px;"><b class="ms-3 me-3"><i id="bookmark"
              ...
              <div class="mb-5">
                ...
                <form method="POST"
                  ...
                  <div class="input-group d-flex justify-content-between" style="">
                    <input type="text" id="reply" name="ulasan" maxlength="255" {{-- $userLog === 1 ? 'disabled' : '' --}}
            ...
            <div class="card mb-5" style="border-radius:15px;">
              <div class="card-body">
                <div class="row">
                  <div class="d-flex">
                    @if ($review->user->foto)
                      <img src="{{ asset('storage/' . $review->user->foto) }}" alt="" width="30px"
                    ...
                    <div class="d-flex ml-auto">
                      @if (Auth::check())
                        @if (Auth::user()->id === $review->user->id)
                          ...
                          <svg data-toggle="modal" data-target="#modalEditUlasan{{ $review->id }}" width="23"
                            height="23" viewBox="0 0 28 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                ...
                                <div class="modal-body">
                                  <form action="{{ route('update.ulasan', $review->id) }}" method="post">
                                    ...
                                    <div class="mb-3">
                                      ...
                                      <input type="text" name="ulasan" id="ulasan" class="form-control"
                                        value="{{ $review->ulasan }}">
                                    </div>
                                    <div class="mb-3">
                                      <label for="rating" class="form-label">Rating</label>
                                      <input type="hidden" value="{{ $review->rating }}" name="rating"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 24 24"
                      onclick="konfirmasi_hapus_ulasanrating({{ $review->id }})">
                      <path fill="#B70404"
        ...
        @if (Auth::check())
          @if (Auth::user()->id == $detail_course->user->id)
            <div id="formBalasUlasan{{ $review->id }}" class="mb-5 mt-3 ml-5 hide" style="border-radius:15px;">
              <form action="{{ route('balas.ulasan', [$detail_course->id, $review->user->id, $review->id]) }}"
                method="post">
                @csrf
                <div class="d-flex justify-content-between">
                  <input type="text" id="reply" name="ulasan" maxlength="255" {{-- $userLog === 1 ? 'disabled' : '' --}}
                    class="form-control me-2" placeholder="Tambahkan Balasan Ulasan...">
                  <button type="submit"
